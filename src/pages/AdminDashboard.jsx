  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 p-6 bg-white rounded-2xl shadow-sm border border-slate-200">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              disabled={refreshing}
              className="px-4 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-lg flex items-center gap-2 hover:bg-slate-50 transition-colors"
            >
              <RefreshCw className={`w-4 h-4 ${refreshing ? 'animate-spin' : ''}`} />
              Refresh
            </button>
            <div className="hidden md:flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-lg">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
          {[
            { label: 'Total Requests', value: stats.total, color: 'blue', icon: <Shield className="w-5 h-5" /> },
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 mb-6">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="relative">
              ...
              {showFilters && (
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-slate-200 z-10 p-2">
                  {['all', 'pending', 'approved', 'rejected'].map(status => (
                    <button
                      key={status}
                      onClick={() => setFilter(status)}
                      className={`w-full text-left px-3 py-2 rounded-md text-sm capitalize ${
                        filter === status ? 'bg-blue-100 text-blue-700' : 'hover:bg-slate-100'
                      }`}
                    >
          ...
          {filter !== 'all' && (
            <div className="mt-4 flex items-center gap-2">
              ...
              <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded-md text-sm capitalize">{filter}</span>
          ...
          {request.status === 'pending' && (    
            <div className="flex gap-3 mt-5 pt-4 border-t border-slate-100">
              ...
              <button
                ...
                disabled={isProcessing}
                className="flex-1 py-2.5 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-lg flex items-center justify-center gap-2 hover:bg-emerald-100 disabled:opacity-50 transition-colors"
              >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
      <div className="bg-white rounded-2xl shadow-lg max-w-2xl w-full p-6 relative">
        <button onClick={onClose} className="absolute top-4 right-4 p-1.5 text-slate-400 hover:text-slate-600 rounded-lg transition-colors">
          <X className="w-5 h-5" />
        </button>
